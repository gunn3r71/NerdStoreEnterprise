    ...
    public class AccountController : BaseController
    {
        ...
        private static long ToUnixEpochDate(DateTime date)
        {
            return (long) Math.Round(
                (date.ToUniversalTime() - new DateTimeOffset(1970, 1, 1, 0, 0, 0, 0, TimeSpan.Zero)).TotalSeconds);
        }
        ...
        private async Task<ResponseMessage> RegisterCustomer(UserRegisterViewModel userModel)
        {
            ...
            var createdUserIntegrationEvent =
                new CreatedUserIntegrationEvent(Guid.Parse(user.Id), userModel.Name, user.Email, userModel.Cpf);

            try
            {
                _logger.LogInformation(
                    $"Sending {createdUserIntegrationEvent.GetType()} - Type: {createdUserIntegrationEvent.MessageType}");
                return await _bus.RequestAsync<CreatedUserIntegrationEvent, ResponseMessage>(
                    createdUserIntegrationEvent);
            }
            catch (Exception ex)
            {
